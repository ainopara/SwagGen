...
Each operation lives under the `TestSpec` namespace and within an optional tag: `TestSpec(.tagName).operationId`. If an operation doesn't have an operationId one will be generated from the path and method.
...
let getUserRequest = TestSpec.User.GetUser.Request(id: 123)
...
Each [Request](#request) also has a `makeRequest` convenience function that uses `TestSpec.shared`.
...
There are some options to control how invalid JSON is handled when decoding and these are available as static properties on `TestSpec`:
